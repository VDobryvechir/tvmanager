...
`;
  ...
  void main(void) {
    vec4 color0 = texture2D(uSampler0, vTextureCoord0);
    vec4 color1 = texture2D(uSampler1, vTextureCoord1);
    vec4 color2 = texture2D(uSampler2, vTextureCoord2);
    vec4 color3 = texture2D(uSampler3, vTextureCoord3);
    gl_FragColor =  color0 ;
  }
`;
...
function getVideoTexture(url, texture) {
  ...
  const videoBlock = {
     ...
     nr: dvno++,
     getTexture: (gl) => {
        console.log("getting textutre", video);
      	if (videoBlock.copyVideo) {
      		updateTexture(gl, videoBlock.texture, videoBlock.video);
    	}
